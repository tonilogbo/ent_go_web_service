...
// QueryBand chains the current query on the "band" edge.
func (aq *AlbumQuery) QueryBand() *BandQuery {
...
// WithBand tells the query-builder to eager-load the nodes that are connected to
// the "band" edge. The optional arguments are used to configure the query builder of the edge.
func (aq *AlbumQuery) WithBand(opts ...func(*BandQuery)) *AlbumQuery {
	query := &BandQuery{config: aq.config}
...
func (aq *AlbumQuery) sqlAll(ctx context.Context) ([]*Album, error) {
	...
	if query := aq.withBand; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		ids := make(map[int]*Album, len(nodes))
		for _, node := range nodes {
			ids[node.ID] = node
			fks = append(fks, node.ID)
			node.Edges.Band = []*Band{}
		}
		var (
			edgeids []int
			edges   = make(map[int][]*Album)
		)
		_spec := &sqlgraph.EdgeQuerySpec{
			...
			Predicate: func(s *sql.Selector) {
				s.Where(sql.InValues(album.BandPrimaryKey[1], fks...))
			},
		...
		if err := sqlgraph.QueryEdges(ctx, aq.driver, _spec); err != nil {
			return nil, fmt.Errorf(`query edges "band": %w`, err)
		}
		...
		for _, n := range neighbors {
			nodes, ok := edges[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected "band" node returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.Band = append(nodes[i].Edges.Band, n)
			}
		}
	}
	...
	if query := aq.withArtist; query != nil {
		...
		_spec := &sqlgraph.EdgeQuerySpec{
			...
			Predicate: func(s *sql.Selector) {
				s.Where(sql.InValues(album.ArtistPrimaryKey[1], fks...))
			},
			ScanValues: func() [2]interface{} {
				return [2]interface{}{new(sql.NullInt64), new(sql.NullInt64)}
			},
		...
		if err := sqlgraph.QueryEdges(ctx, aq.driver, _spec); err != nil {
			return nil, fmt.Errorf(`query edges "artist": %w`, err)
		}
		...
		for _, n := range neighbors {
			nodes, ok := edges[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected "artist" node returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.Artist = append(nodes[i].Edges.Artist, n)
			}
		}
	}
