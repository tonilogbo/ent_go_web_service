...
// The BandFunc type is an adapter to allow the use of ordinary
// function as Band mutator.
type BandFunc func(context.Context, *ent.BandMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f BandFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	mv, ok := m.(*ent.BandMutation)
	if !ok {
		return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.BandMutation", m)
	}
	return f(ctx, mv)
}
