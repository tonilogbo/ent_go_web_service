...
type LabelQuery struct {
	...
	// eager-loading edges.
	withIndividualArtists *ArtistQuery
	withBands             *BandQuery
...
// QueryIndividualArtists chains the current query on the "individual_artists" edge.
func (lq *LabelQuery) QueryIndividualArtists() *ArtistQuery {
	query := &ArtistQuery{config: lq.config}
...
func (lq *LabelQuery) QueryBands() *BandQuery {
	query := &BandQuery{config: lq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := lq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (lq *LabelQuery) Clone() *LabelQuery {
	...
	return &LabelQuery{
		...
		withIndividualArtists: lq.withIndividualArtists.Clone(),
		withBands:             lq.withBands.Clone(),
...
// WithIndividualArtists tells the query-builder to eager-load the nodes that are connected to
// the "individual_artists" edge. The optional arguments are used to configure the query builder of the edge.
func (lq *LabelQuery) WithIndividualArtists(opts ...func(*ArtistQuery)) *LabelQuery {
	query := &ArtistQuery{config: lq.config}
	for _, opt := range opts {
		opt(query)
	}
	lq.withIndividualArtists = query
	return lq
}
...
func (lq *LabelQuery) WithBands(opts ...func(*BandQuery)) *LabelQuery {
	query := &BandQuery{config: lq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (lq *LabelQuery) sqlAll(ctx context.Context) ([]*Label, error) {
	...
	if query := lq.withIndividualArtists; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		ids := make(map[int]*Label, len(nodes))
		for _, node := range nodes {
			ids[node.ID] = node
			fks = append(fks, node.ID)
			node.Edges.IndividualArtists = []*Artist{}
		}
		var (
			edgeids []int
			edges   = make(map[int][]*Label)
		)
		_spec := &sqlgraph.EdgeQuerySpec{
			...
			Predicate: func(s *sql.Selector) {
				s.Where(sql.InValues(label.IndividualArtistsPrimaryKey[0], fks...))
			},
		...
		if err := sqlgraph.QueryEdges(ctx, lq.driver, _spec); err != nil {
			return nil, fmt.Errorf(`query edges "individual_artists": %w`, err)
		}
		...
		for _, n := range neighbors {
			nodes, ok := edges[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected "individual_artists" node returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.IndividualArtists = append(nodes[i].Edges.IndividualArtists, n)
			}
		}
	}

	if query := lq.withBands; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		ids := make(map[int]*Label, len(nodes))
		for _, node := range nodes {
			ids[node.ID] = node
		...
		_spec := &sqlgraph.EdgeQuerySpec{
			...
			Predicate: func(s *sql.Selector) {
				s.Where(sql.InValues(label.BandsPrimaryKey[0], fks...))
			},
			ScanValues: func() [2]interface{} {
				return [2]interface{}{new(sql.NullInt64), new(sql.NullInt64)}
			},
